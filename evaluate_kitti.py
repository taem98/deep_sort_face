...
def parse_args():
    ...
    parser.add_argument(
        "--kitti_dir", help="Path to Kitti Challenge directory (train or test)",
        required=True)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "be created if it does not exist.", default="./results/vgg16_notop")
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    sequences = os.listdir(args.kitti_dir)
    args_file = os.path.join(args.output_dir, "args.json")
    with open(args_file, 'w') as f:
        json.dump(vars(args), f, ensure_ascii=False, indent=2, sort_keys=True)
    for sequence in sequences:
        print("Running sequence %s" % sequence)
        sequence_dir = os.path.join(args.kitti_dir, sequence)
